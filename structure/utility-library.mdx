---
title: "aoc_cj.util: Shared Utilities for Puzzle Solutions"
sidebarTitle: "Utility Library"
description: "Reference for all public exports from aoc_cj.util: grid adjacency helpers, regex-based input parsers, is_prime, Point3D, and the generic PriorityQueue."
---
...
from aoc_cj.util import adj_4, adj_8, ints, positive_ints, is_prime, PriorityQueue, Point3D
```
...
Extracts only non-negative integers (no leading minus sign) using `r'\d+'`.
...
Parses a `"start-stop"` string (inclusive on both ends) into a Python `range`. Uses `positive_ints` internally.
...
| `positive_ints` | `(str) -> Generator[int]` | Extract non-negative integers from a string |
| `is_prime` | `(int) -> bool` | Trial-division primality test |
| `parse_range` | `(str) -> range` | Parse `"start-stop"` into an inclusive `range` |
